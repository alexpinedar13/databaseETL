...
@st.cache_data
def cargar_datos():
    ruta_archivo = os.path.join(RUTA_PROCESADO, "ventas_enriquecidas.csv")
    ruta_hash = os.path.join(RUTA_PROCESADO, "dataset_hash.txt")
...
if df is not None:
    ...
    st.error("No se encontró el archivo de datos procesados. Por favor, ejecuta `src/etl.py` primero para generar los datos.")
